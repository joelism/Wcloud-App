...
// Wheel-Modus & Namens-Glücksrad
let wheelMode = "regular"; // "regular" oder "nameWheel"
let wheelNamesAll = [];
...
function spinNameWheel(){
  ...
  // Schon angezeigte URLs herausfiltern, damit nicht immer die gleichen kommen
  const usedUrls = new Set(wheelDisplayed.map(e => e.url));
  const unused = matchingEntries.filter(e => !usedUrls.has(e.url));

  const pool = unused.length ? unused : matchingEntries; // falls alles schon verwendet wurde

  // Genau EIN Bild zufällig auswählen
  const imgIdx = Math.floor(Math.random() * pool.length);
  const chosenEntry = pool[imgIdx];

  // Nur dieses eine Bild hinzufügen
