// Wcloud – kompletter App-Code mit Startscreen & wcloud-wheel
// Service Worker werden deaktiviert, du brauchst keine sw.js mehr.

// --- Service Worker deaktivieren (alte Registrierungen entfernen) ---
...
function doLogin(ev) {
  ...
  try {
    ...
    if (val === PIN) {
      ...
      if (app) app.hidden = false;    // zeigt Startscreen + App
      return false;
    } else {
      if (err) {
        err.hidden = false;
...
function activateTab(tabId) {
  ...
  panels.forEach(panel => {
    if (panel.id === tabId || (tabId === "capture" && panel.id === "startScreen" && false)) {
      panel.classList.remove("hidden");
    } else if (panel.id !== "startScreen") {
...
// Startscreen: Einträge & Analyse / wcloud-wheel
function setupStartScreen() {
  ...
  // Beim Laden: Startscreen sichtbar, Tabs versteckt
...
// ---------- wcloud-wheel (Random-Bilder-Pool) ----------
let wcloudImageUrls = [];

// Kannst du später z.B. aus Google-Drive-Code aufrufen:
...
function renderWcloudWheelRandom() {
  ...
  if (!wcloudImageUrls.length) {
    grid.innerHTML = "<p class='muted'>Noch keine Bilder geladen. Pool ist leer.</p>";
    return;
  }
