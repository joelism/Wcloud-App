...
// Struktur: [{ url: string, name: string }]
let imageUrls = [];
let wheelDisplayed = [];   // aktuell in der Galerie (Array von {url,name})
let wheelRemaining = [];   // für regulären Zufallsmodus
...
// Einen Namen per Glücksrad ziehen und genau EIN Bild hinzufügen
function spinNameWheel(){
  ...
  // Zufällig einen Namen aus dem verbleibenden Pool ziehen
  ...
  // Alle Bilder mit diesem Namen suchen
  ...
  // Schon angezeigte URLs herausfiltern
...
// Lightbox (geöffnete Bildansicht, vertikales Swipen)
let lightboxIndex = -1;
...
function setupLightbox(){
  ...
  // Vertikales Swipen wie Instagram-Storys (oben/unten)
...
async function importBackupFile(file){
  ...
  if (Array.isArray(data.imageUrls)){
    imageUrls = data.imageUrls.map(item=>{
      if (typeof item === "string"){
        const url = item.trim();
        return url ? { url, name: "" } : null;
      }
