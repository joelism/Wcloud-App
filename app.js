...
function openDB() {
  return new Promise((resolve, reject) => {
    const req = indexedDB.open("PrivateTrackerDB", 1);
    req.onupgradeneeded = () => {
      const db = req.result;
...
function setupAgeGate() {
  ...
  const confirmBtn = $("#ageConfirm");
  const declineBtn = $("#ageDecline");
  const ok = localStorage.getItem("ageOk") === "yes";
  if (!ok) ageGate.style.display = "flex";

  confirmBtn.addEventListener("click", () => {
    localStorage.setItem("ageOk", "yes");
    ageGate.style.display = "none";
    $("#app").hidden = false;
  });
  declineBtn.addEventListener("click", () => {
    alert("Zugang verweigert. Diese App ist nur für Volljährige (18+).");
  });
  if (ok) $("#app").hidden = false;
}
...
function renderHistoryList(list) {
  ...
  list.forEach(s => {
    ...
    main.innerHTML = `<div><strong>${s.isoDate}</strong> ${d.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</div>
      <div class="meta">${s.content} · ${s.gender} · ${s.porn} · ${s.wet}</div>`;
