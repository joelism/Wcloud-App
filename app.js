...
function appendWheelImages(entries, replace){
  ...
  entries.forEach(entry=>{
    ...
    // Klick auf das Item öffnet die Lightbox an der entsprechenden Position
...
function renderWheelTagFilter(){
  ...
  imageTags.forEach(tag=>{
    ...
    btn.addEventListener("click", ()=>{
      ...
      renderWheelTagFilter();
      rebuildWheelPool();
      // Bestehende Bilder bleiben stehen – nur der Pool für zukünftige Zufallsbilder ändert sich.
      if (!wheelDisplayed.length){
        renderWheelReset();
      } else {
        updateWheelInfo("Tag-Auswahl geändert – neue zufällige Bilder beachten die aktuelle Auswahl.");
      }
    });
  ...
  clearBtn.addEventListener("click", ()=>{
    ...
    renderWheelTagFilter();
    rebuildWheelPool();
    // Bestehende Bilder bleiben stehen – Info nur aktualisieren, falls noch nichts angezeigt wird.
    if (!wheelDisplayed.length){
      renderWheelReset();
    } else {
      updateWheelInfo("Tag-Filter zurückgesetzt – alle Bilder werden wieder für neue Zufallsbilder verwendet.");
    }
  });
...
function spinNameWheel(){
  ...
  // Angezeigten Namen groß & präsent darstellen
  const current = $("#wheelCurrentName");
  if (current){
  ...
  // Bild für diesen Namen anhängen, ohne nach unten zu scrollen
  const prevScrollY = (window.scrollY ?? window.pageYOffset ?? 0);

  const base = getBaseImagesForWheel();
  let entry = null;
  if (base.length){
    const normalizedChosen = normalizeName(chosen);
    const candidates = base.filter(e => normalizeName(e.name) === normalizedChosen);
...
function setupWcloudWheel(){
    ...
    modeRadios.forEach(radio=>{
      radio.addEventListener("change",(ev)=>{
        ...
        if (wheelMode === "regular"){
          ...
          // Bereits angezeigte Bilder bleiben erhalten – nur der Pool für neue Zufallsbilder ändert sich.
  ...
  // Initialen UI-Zustand anhand der aktuellen Auswahl setzen
  updateWheelModeUI();
}
...
function setupSettings(){
  ...
  // --- Untermenü-Navigation im Einstellungs-Tab (Backup / Tags / Bilder) ---
  const settingsPanel = $("#settings");
  if (settingsPanel) {
    const cards = settingsPanel.querySelectorAll(".card");
    const backupCard = cards[0] || null;
    const poolCard = cards[1] || null;

    // Nur wenn beide Karten existieren, lohnt sich die Unterteilung
    if (backupCard && poolCard) {
      // Subnav-Container erstellen
      const subnav = document.createElement("div");
      subnav.id = "settingsSubnav";
      subnav.className = "row wrap";
      subnav.style.marginBottom = "0.9rem";
      subnav.style.gap = "0.5rem";

      function makeSubnavButton(label){
        const btn = document.createElement("button");
        btn.type = "button";
      ...
      const backupBtn = makeSubnavButton("Backup");
      const tagsBtn = makeSubnavButton("Tags");
      const imagesBtn = makeSubnavButton("Bilder");

      subnav.appendChild(backupBtn);
      subnav.appendChild(tagsBtn);
      subnav.appendChild(imagesBtn);

      // Subnav ganz oben im Settings-Panel einfügen
      settingsPanel.insertBefore(subnav, settingsPanel.firstElementChild);

      // Elemente innerhalb der Bild-Pool-Karte für die Bereiche "Tags" und "Bilder"
      const tagsHeading = poolCard.querySelector("h3");
      const mutedParas = poolCard.querySelectorAll("p.muted");
      const tagsDescription = mutedParas.length > 1 ? mutedParas[1] : null;

      const newTagInputEl = $("#newTagInput");
      const tagRow = newTagInputEl ? newTagInputEl.closest(".row") : null;
      const tagListManage = $("#tagListManage");

      const imageUrlInputEl = $("#imageUrlInput");
      const imageRow = imageUrlInputEl ? imageUrlInputEl.closest(".row") : null;
      const imagePoolInfo = $("#imagePoolInfo");
      const imagePoolGrid = $("#imagePoolGrid");

      function setSection(section){
        // Button-Styles
        const active = "btn btn-primary btn-small";
        const inactive = "btn btn-secondary btn-small";
        backupBtn.className = section === "backup" ? active : inactive;
        tagsBtn.className = section === "tags" ? active : inactive;
        imagesBtn.className = section === "images" ? active : inactive;
        ...
        if (section === "backup"){
          ...
          if (tagsHeading) tagsHeading.style.display = "";
          if (tagsDescription) tagsDescription.style.display = "";
          if (tagRow) tagRow.style.display = "";
          if (tagListManage) tagListManage.style.display = "";

          if (imageRow) imageRow.style.display = "none";
          if (imagePoolInfo) imagePoolInfo.style.display = "none";
          if (imagePoolGrid) imagePoolGrid.style.display = "none";
        } else if (section === "images"){
          if (backupCard) backupCard.style.display = "none";
          if (poolCard) poolCard.style.display = "";

          if (tagsHeading) tagsHeading.style.display = "none";
          if (tagsDescription) tagsDescription.style.display = "none";
          if (tagRow) tagRow.style.display = "none";
          if (tagListManage) tagListManage.style.display = "none";

          if (imageRow) imageRow.style.display = "";
          if (imagePoolInfo) imagePoolInfo.style.display = "";
          if (imagePoolGrid) imagePoolGrid.style.display = "";
        }
      }

      // Klick-Handler
      backupBtn.addEventListener("click", ()=>setSection("backup"));
      tagsBtn.addEventListener("click", ()=>setSection("tags"));
      imagesBtn.addEventListener("click", ()=>setSection("images"));

      // Standard-Ansicht: Backup
      setSection("backup");
    }
  }
