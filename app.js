...
function appendWheelImages(entries, replace){
  ...
  entries.forEach(entry=>{
    ...
    // Klick auf das gesamte Item öffnet die Lightbox und erlaubt Hoch/Runter-Swipen
...
function renderWheelTagFilter(){
  ...
  imageTags.forEach(tag=>{
    ...
    btn.addEventListener("click", ()=>{
      ...
      renderWheelTagFilter();
      // Nur den Pool für zukünftige Zufallsbilder aktualisieren,
      // aber die bereits angezeigten Bilder NICHT löschen.
      wheelRemaining = getBaseImagesForWheel();
    });
  ...
  clearBtn.addEventListener("click", ()=>{
    selectedTags.clear();
    renderWheelTagFilter();
    // Auch hier nur den Pool neu aufbauen,
    // bereits dargestellte Bilder bleiben erhalten.
    wheelRemaining = getBaseImagesForWheel();
  });
...
function spinNameWheel(){
  // Namengenerator: zieht einen Namen und zeigt ein passendes Bild unten an
  if (!wheelNamesAll.length){
    alert("Keine Namen im Pool. Bitte zuerst bei den Bild-URLs Namen vergeben.");
    return;
  }

  // Liste der noch nicht gezogenen Namen auffüllen, falls leer
  ...
  if (matching.length){
    ...
    updateWheelInfo("Keine Bilder für die aktuelle Tag-Auswahl vorhanden.");
    return;
  }

  // Bild unten anhängen (Lightbox funktioniert damit automatisch mit)
  ...
  // Visuelle Namensliste aktualisieren
  renderWheelNamesList();
}
