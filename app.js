...
function renderTagManagement(){
  ...
  if (!imageTags.length){
    const p = document.createElement("p");
    p.className = "muted";
    p.textContent = "Noch keine Tags vorhanden. Du kannst neue Tags oben anlegen oder direkt bei einem Bild setzen.";
    list.appendChild(p);
    return;
  }
...
function renderImagePool(){
  ...
  imageUrls.forEach((entry, idx)=>{
    ...
    tagBtn.textContent = "🏷";
    tagBtn.title = "Tags bearbeiten";
    tagBtn.style.border = "none";
    tagBtn.style.background = "transparent";
    tagBtn.style.color = "#9ca3af";
    tagBtn.style.fontSize = "0.75rem";
    tagBtn.style.cursor = "pointer";
    tagBtn.style.padding = "0 0.25rem";
    tagBtn.addEventListener("click",(ev)=>{
      ev.stopPropagation();
      const currentStr = (entry.tags || []).join(", ");
      const input = prompt("Tags für dieses Bild (kommasepariert)", currentStr);
    ...
    tagsRow.style.fontSize = "0.7rem";
    tagsRow.style.marginTop = "0.25rem";
    const tagsText = entry.tags && entry.tags.length
      ? "Tags: " + entry.tags.join(", ")
      : "Keine Tags hinterlegt";
    tagsRow.textContent = tagsText;
