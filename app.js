...
function spinNameWheel(){
  ...
  // Visuales Glücksrad animieren
  const circle = $("#wheelCircle");
  if (circle && wheelNamesAll.length){
    const sector = 360 / wheelNamesAll.length;
    const idxAll = wheelNamesAll.indexOf(name);
    const targetAngle = idxAll * sector + sector / 2;
    wheelRotation = (wheelRotation || 0) + 720 + (360 - targetAngle);
