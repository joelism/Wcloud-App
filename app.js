...
function rebuildWheelPool(){
  // aktualisiert nur den Pool der noch verfügbaren Bilder
  // bereits angezeigte Bilder (wheelDisplayed) bleiben unverändert
...
function renderWheelReset(){
  const grid = $("#wheelGrid");
  if (!grid) return;
  // komplettes Reset: bereits angezeigte Bilder verwerfen
  wheelDisplayed = [];
  rebuildWheelPool();
  if (!wheelRemaining.length){
    grid.innerHTML = "<p class='muted'>Keine Bilder für die aktu...e Tag-Auswahl. Passe den Tag-Filter oder den Bild-Pool an.</p>";
  } else {
    grid.innerHTML = "<p class='muted'>Noch keine Bilder angezei...icke auf „Mehr Bilder laden“ oder benutze das Namen-Wheel.</p>";
  }
}
...
function renderWheelTagFilter(){
  ...
  imageTags.forEach(tag=>{
    ...
    btn.addEventListener("click", ()=>{
      ...
      // Tag-UI aktualisieren
      renderWheelTagFilter();
      // neuen Pool für zukünftige Zufallsbilder aufbauen,
      // aber bereits angezeigte Bilder im Grid nicht löschen
      rebuildWheelPool();
    });
...
function spinNameWheel(){
  // Aktuelle Scroll-Position merken, damit das iPhone nicht nach unten springt.
  const prevScrollY = window.scrollY ?? window.pageYOffset ?? 0;

  // Falls der Namen-Pool leer ist, aus allen vorhandenen Namen initialisieren
  ...
  // Zufälligen Namen ziehen (ohne Wiederholung, bis alle einmal dran waren)
  ...
  const current = $("#wheelCurrentName");
  if (current) {
    current.textContent = chosen || "(kein Name)";
    // Große, präsente Darstellung des gezogenen Namens
    current.style.fontSize = "2rem";
    current.style.fontWeight = "900";
    current.style.textAlign = "center";
    current.style.marginTop = "1.2rem";
    current.style.color = "#ffffff";
    current.style.letterSpacing = "0.6px";
    current.style.lineHeight = "1.2";
    current.style.opacity = "1";
    current.style.transition = "transform 0.25s ease, opacity 0.25s ease";
  ...
  // Passende Bilder zum gezogenen Namen + aktueller Tag-Auswahl suchen
  const base = getBaseImagesForWheel();
  const matching = base.filter(e => normalizeName(e.name) === chosen);

  let entry = null;
  if (matching.length){
    entry = matching[Math.floor(Math.random() * matching.length)];
  } else if (base.length){
    // Fallback: irgendein Bild aus der aktuellen Auswahl
    entry = base[Math.floor(Math.random() * base.length)];
  }

  if (entry){
    // Bild unten im Grid anhängen (kein automatisches Scrollen)
    appendWheelImages([entry], false);
  }

  // Namen-Labels rund ums Wheel aktualisieren
  updateVisualWheel();
  renderWheelNamesList();

  // Scroll-Position wiederherstellen (gegen iOS-Auto-Scroll)
  try {
...
function setupWcloudWheel(){
    ...
    modeRadios.forEach(radio=>{
      ...
      radio.addEventListener("change",(ev)=>{
        ...
        if (wheelMode === "regular"){
          ...
          // vorhandene angezeigte Bilder bleiben stehen
  ...
  // Initial UI-Zustand entsprechend der aktuellen wheelMode
  updateWheelModeUI();
}
...
function setupSettings(){
  ...
  // Untermenüs für Einstellungen (Backup / Tags / Bilder)
  const settingsPanel = $("#settings");
  if (settingsPanel){
    const cards = settingsPanel.querySelectorAll(".card");
    if (cards.length >= 2){
      const backupCard = cards[0];
      const poolCard = cards[1];

      // Elemente für Tags & Bilder identifizieren
      const tagList = $("#tagListManage");
      const newTagInput = $("#newTagInput");
      const imageUrlInput = $("#imageUrlInput");
      const imagePoolInfo = $("#imagePoolInfo");
      const imagePoolGrid = $("#imagePoolGrid");

      const tagElements = [];
      if (poolCard){
        const headings = poolCard.querySelectorAll("h3");
        headings.forEach(h=>{
          if (h.textContent && h.textContent.toLowerCase().includes("tags")){
            tagElements.push(h);
            const p = h.nextElementSibling;
            if (p && p.tagName === "P") tagElements.push(p);
          }
        });
      }
      if (newTagInput){
        const row = newTagInput.closest(".row") || newTagInput.parentElement;
        if (row) tagElements.push(row);
      }
      if (tagList) tagElements.push(tagList);

      const imageElements = [];
      if (imageUrlInput){
        const row = imageUrlInput.closest(".row") || imageUrlInput.parentElement;
        if (row) imageElements.push(row);
      }
      if (imagePoolInfo) imageElements.push(imagePoolInfo);
      if (imagePoolGrid) imageElements.push(imagePoolGrid);

      let btnBackup, btnTags, btnImages;

      function setSection(section){
        if (!backupCard || !poolCard) return;
        ...
        if (section === "backup"){
          ...
          tagElements.forEach(el=>{
            if (!el) return;
            el.style.display = showTags ? "" : "none";
          });
        ...
        [btnBackup, btnTags, btnImages].forEach(btn=>{
          if (!btn) return;
          btn.classList.remove("btn-primary");
          btn.classList.add("btn-secondary");
        });
        if (section === "backup" && btnBackup){
          btnBackup.classList.remove("btn-secondary");
          btnBackup.classList.add("btn-primary");
        } else if (section === "tags" && btnTags){
          btnTags.classList.remove("btn-secondary");
          btnTags.classList.add("btn-primary");
        } else if (section === "images" && btnImages){
          btnImages.classList.remove("btn-secondary");
          btnImages.classList.add("btn-primary");
        }
      }

      // Nav-Leiste nur einmal einfügen
      let nav = settingsPanel.querySelector("#settingsSubnav");
      if (!nav){
        nav = document.createElement("div");
        nav.id = "settingsSubnav";
        ...
        btnBackup.className = "btn btn-secondary btn-small";

        btnTags = document.createElement("button");
        btnTags.type = "button";
        btnTags.textContent = "Tags";
        btnTags.className = "btn btn-secondary btn-small";

        btnImages = document.createElement("button");
        btnImages.type = "button";
        btnImages.textContent = "Bilder";
        btnImages.className = "btn btn-secondary btn-small";

        btnBackup.addEventListener("click", ()=>setSection("backup"));
        btnTags.addEventListener("click", ()=>setSection("tags"));
        btnImages.addEventListener("click", ()=>setSection("images"));

        nav.appendChild(btnBackup);
        nav.appendChild(btnTags);
        nav.appendChild(btnImages);

        settingsPanel.insertBefore(nav, backupCard);
      }

      // Standard: Backup anzeigen
      setSection("backup");
    }
  }
}
...
async function exportBackup(){
  ...
  // Bild-URLs beim Export immer mit URL, Name und Tags speichern
  const exportedImageUrls = imageUrls.map(item => {
    if (typeof item === "string") {
      const url = item.trim();
      return url ? { url, name: DEFAULT_IMAGE_NAME, tags: [] } : null;
    }
